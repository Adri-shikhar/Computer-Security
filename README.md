...
│   │                                                     │     │
│   │   Step 1: NO Salt Generated (Educational Demo)     │     │
│   │   salt = ""  (empty string)                         │     │
│   │   ⚠️ Intentionally weak for demonstration          │     │
│   │                                                     │     │
│   │   Step 2: Hash with MD5 Only                        │     │
│   │   hash = hashlib.md5("abc".encode()).hexdigest()    │     │
│   │        = "900150983cd24fb0d6963f7d28e17f72"         │     │
│   │   (No salt added - same password = same hash)       │     │
│   │                                                     │     │
...
│                                                               │
│   ⚠️ SECURITY NOTE:                                           │
│   - Two users with "abc" = IDENTICAL hash (vulnerable!)      │
│   - No salt = Vulnerable to rainbow table attacks            │
│   - MD5 intentionally weak for educational demonstration     │
│   - Use Salt & Hash Manager to upgrade to secure algorithms  │
│                                                               │
└──────────────────────────────────────────────────────────────┘
```
...
│   User Browser                                                   │
│   ┌──────────────────────────────────────────────────────┐      │
│   │ index.html → form input (name, email, password)      │      │
│   └────────────────────┬─────────────────────────────────┘      │
...
│   │   1. Receive JSON data                               │      │
│   │   2. Validate input                                  │      │
│   │   3. Hash password (MD5, no salt)                    │      │
│   │   4. Calculate security score                        │      │
│   │   5. Check breach status (HIBP API)                  │      │
│   └────────────────────┬─────────────────────────────────┘      │
│                        │ SQL INSERT                             │
│                        ▼                                         │
│   SQLite Database                                                │
...
│   ┌──────────────────────────────────────────────────────┐      │
│   │ GET /api/users → Display table with:                 │      │
│   │   - User details                                     │      │
│   │   - Hash values                                      │      │
│   │   - Security scores                                  │      │
...
│   │                                                      │      │
│   │ Audit Tools:                                         │      │
│   │   → Find Duplicates   (/api/audit/duplicate-*)      │      │
│   │   → Weak Passwords    (/api/audit/weak-*)           │      │
│   │   → Breached Check    (/api/audit/breached-*)       │      │
│   │   → Hash Distribution (/api/audit/hash-dist*)        │      │
│   └──────────────────────────────────────────────────────┘      │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────────┐
│                    HASH MIGRATION PROCESS                        │
│           (Salt & Hash Manager - 3-in-1 Tool)                   │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│   SCENARIO: Upgrade MD5 → SHA-256 with 256-bit Salt             │
│                                                                  │
│   Before Migration:                                              │
│   ┌──────────────────────────────────────────────────────┐      │
...
│   │ Algorithm: MD5                                       │      │
│   │ Salt: "" (empty)                                     │      │
│   │ Hash: 900150983cd24fb0d6963f7d28e17f72 (MD5 of "abc")│      │
│   │ Security Score: 15 (WEAK)                            │      │
│   └─────────────────────┬────────────────────────────────┘      │
│                         │                                        │
│                         │ User selects migration options         │
│                         │                                        │
│                         ▼                                        │
│   ┌──────────────────────────────────────────────────────┐      │
│   │ POST /api/hash-migration/convert                     │      │
│   │                                                      │      │
│   │ Request Body:                                        │      │
│   │ {                                                    │      │
│   │   "user_id": 1,                                      │      │
│   │   "target_algorithm": "sha256",                      │      │
│   │   "salt_length": 32  // bytes (256-bit)             │      │
│   │ }                                                    │      │
│   └─────────────────────┬────────────────────────────────┘      │
│                         │                                        │
│                         ▼                                        │
│   ┌──────────────────────────────────────────────────────┐      │
│   │ MIGRATION LOGIC (Backend)                            │      │
│   │                                                      │      │
│   │ Step 1: Generate New Salt                            │      │
│   │   new_salt = secrets.token_hex(32)                   │      │
│   │   = "a1b2...f0e1" (64 hex chars = 32 bytes)          │      │
│   │                                                      │      │
│   │ Step 2: Combine Old Hash + New Salt                  │      │
│   │   input = old_hash + new_salt                        │      │
│   │   = "900150983cd24fb0..." + "a1b2...f0e1"            │      │
│   │                                                      │      │
│   │ Step 3: Hash with New Algorithm                      │      │
│   │   new_hash = sha256(input.encode()).hexdigest()      │      │
│   │   = "8a7b6c5d4e3f2a1b..." (SHA-256 output)           │      │
│   │                                                      │      │
│   │ Step 4: Update Security Score                        │      │
│   │   new_score = 85  (Strong - upgraded!)              │      │
│   └─────────────────────┬────────────────────────────────┘      │
│                         │                                        │
│                         ▼                                        │
│   ┌──────────────────────────────────────────────────────┐      │
...
│   │   algorithm = 'SHA256',                              │      │
│   │   salt = 'a1b2...f0e1',                              │      │
│   │   password_hash = '8a7b6c5d4e3f2a1b...',             │      │
│   │   security_score = 85                                │      │
│   │ WHERE id = 1                                         │      │
│   └─────────────────────┬────────────────────────────────┘      │
│                         │                                        │
...
│   │ User: john@example.com                               │      │
│   │ Algorithm: SHA256 ✅                                 │      │
│   │ Salt: a1b2...f0e1 (256-bit) ✅                       │      │
│   │ Hash: 8a7b6c5d4e3f2a1b... (SHA-256) ✅               │      │
│   │ Security Score: 85 (STRONG) ✅                       │      │
│   └──────────────────────────────────────────────────────┘      │
│                                                                  │
│   ⚠️ IMPORTANT NOTES:                                            │
│   • User's original password UNCHANGED                           │
│   • Only storage security upgraded                              │
│   • User can still login with same password                     │
│   • Migration is irreversible (old hash lost)                   │
...
```
┌─────────────────────────────────────────────────────────────────┐
│              BATCH HASH MIGRATION PROCESS                        │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│   Scenario: Migrate all MD5 users → SHA-256 with salt           │
│                                                                  │
│   Step 1: Identify Target Users                                 │
│   ┌──────────────────────────────────────────────────────┐      │
│   │ GET /api/hash-migration/users                        │      │
│   │                                                      │      │
│   │ Returns users filtered by:                           │      │
│   │   - algorithm = "MD5"                                │      │
│   │   - salt = "" (empty)                                │      │
│   │   - security_score < 50                              │      │
│   │                                                      │      │
...
│                         │                                        │
│                         ▼                                        │
│   Step 2: Select Users & Options                                │
│   ┌──────────────────────────────────────────────────────┐      │
│   │ UI: Salt & Hash Manager                              │      │
│   │                                                      │      │
│   │ ☑ john@example.com     (MD5 → SHA-256)               │      │
│   │ ☑ jane@example.com     (MD5 → SHA-256)               │      │
│   │ ☑ bob@example.com      (MD5 → SHA-256)               │      │
│   │                                                      │      │
│   │ Target Algorithm: [SHA-256 ▼]                        │      │
│   │ Salt Length: [32 bytes (256-bit) ▼]                  │      │
│   │                                                      │      │
│   │ [Migrate Selected Users] button                      │      │
│   └─────────────────────┬────────────────────────────────┘      │
│                         │                                        │
│                         ▼                                        │
│   Step 3: Batch Migration API Call                              │
│   ┌──────────────────────────────────────────────────────┐      │
│   │ POST /api/hash-migration/batch-convert               │      │
│   │                                                      │      │
│   │ {                                                    │      │
│   │   "user_ids": [1, 2, 3],                             │      │
│   │   "target_algorithm": "sha256",                      │      │
...
│                         │                                        │
│                         ▼                                        │
│   Step 4: Backend Processing (Loop)                             │
│   ┌──────────────────────────────────────────────────────┐      │
│   │ FOR EACH user_id:                                    │      │
│   │   1. Generate unique salt                            │      │
│   │   2. Combine old_hash + new_salt                     │      │
│   │   3. Hash with SHA-256                               │      │
│   │   4. Update database record                          │      │
│   │   5. Log migration event                             │      │
│   │                                                      │      │
│   │ Transaction: COMMIT all or ROLLBACK                  │      │
│   └─────────────────────┬────────────────────────────────┘      │
│                         │                                        │
│                         ▼                                        │
...
│   │   • bob@example.com   (MD5 → SHA-256)                │      │
│   │                                                      │      │
│   │ New Security Scores: 85/100 (Strong)                 │      │
│   │ Salt Length: 256-bit                                 │      │
│   └──────────────────────────────────────────────────────┘      │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
...
│   │                                                      │      │
│   │ Result:                                              │      │
│   │   Hash: 900150983cd24fb0...                          │      │
│   │   Users: [john@ex.com, jane@ex.com] (2 affected)    │      │
│   │   ⚠️ Same password = security risk!                  │      │
│   └──────────────────────────────────────────────────────┘      │
│                                                                  │
│   ┌─────────────────────────────────────┐                       │
...
│   │                                                      │      │
│   │ Result:                                              │      │
│   │   john@ex.com    Score: 15 (Very Weak)              │      │
│   │   bob@ex.com     Score: 35 (Weak)                    │      │
│   │   jane@ex.com    Score: 48 (Weak)                    │      │
│   └──────────────────────────────────────────────────────┘      │
│                                                                  │
│   ┌─────────────────────────────────────┐                       │
...
│   │   - Common passwords list (local)                    │      │
│   │   - HIBP API (online breach database)                │      │
│   │                                                      │      │
│   │ Result:                                              │      │
│   │   admin@ex.com   Password: "password123" 🔴          │      │
│   │   Found in 2.3M breaches (HIBP)                      │      │
│   └──────────────────────────────────────────────────────┘      │
│                                                                  │
│   ┌─────────────────────────────────────┐                       │
...
│   │   MD5:     75 users (75%) 🔴                         │      │
│   │   SHA-256:  15 users (15%) 🟡                        │      │
│   │   Argon2id: 10 users (10%) 🟢                        │      │
│   │                                                      │      │
│   │ Security Levels:                                     │      │
│   │   Weak (0-50):     60 users (60%)                    │      │
│   │   Moderate (51-70): 25 users (25%)                   │      │
│   │   Strong (71-100):  15 users (15%)                   │      │
│   │                                                      │      │
│   │ Recommendation:                                      │      │
│   │   → Migrate 75 MD5 users to SHA-256/Argon2id        │      │
│   └──────────────────────────────────────────────────────┘      │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```
